...
def index():
    ...
    for v in runners.locust_runner.locust_classes:
        tasks.extend(v.task_set.tasks) 
    tasks = list(set(tasks))
    print tasks
    for value in tasks:
       tasks_str = tasks_str + " " + value.__name__
    print 'index:', tasks_str
    if is_distributed:
...
#start_methods = ' '.join(runners.all_tasks[0].__name__)
#    print start_methods
#start_method = request.form["start_method"].split()
    print 'swarm', request.form["start_method"]
    runners.locust_runner.choose_method(request.form["start_method"])
